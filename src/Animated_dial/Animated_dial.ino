...
#define NEEDLE_LENGTH 20  // Visible length
#define NEEDLE_WIDTH   5  // Width of needle - make it an odd number
#define NEEDLE_RADIUS 115  // Radius at tip
...
void setup()   {
  ...
  // Draw the dial
  //TJpgDec.drawFsJpg(0, 0, dial, sizeof(dial));
  drawBackground();
  ...
  // Plot the label text
  tft.setTextColor(TFT_BLACK,  tft.color565(29, 155, 29));
  tft.setTextDatum(MC_DATUM);
  tft.drawString("RPM", DIAL_CENTRE_X, DIAL_CENTRE_Y + 68, 2);
...
void loop() {
  ...
  tft.setTextColor(TFT_BLACK, tft.color565(29, 155, 29));
  tft.setTextDatum(MC_DATUM);
  tft.drawString(String(rpm), DIAL_CENTRE_X, DIAL_CENTRE_Y + 88, 2);
...
void drawBackground(void) {
  rc = png.openFLASH((uint8_t *)dial, sizeof(dial), pngDraw);
  if (rc == PNG_SUCCESS) {
  ...
  while (angle != old_angle || !buffer_loaded) {
    ...
    if ( (old_angle & 1) == 0)
    {
      if (buffer_loaded) {
        ...
        //tft.pushRect(min_x - 1, min_y - 1, 1 + max_x - min_x, 1 + max_y - min_y, tft_buffer);
      }

      if ( needle.getRotatedBounds(old_angle, &min_x, &min_y, &max_x, &max_y) )
      {
        // Grab a copy of the area before needle is drawn
        tft.readRect(min_x - 1, min_y - 1, 1 + max_x - min_x, 1 + max_y - min_y, tft_buffer);
        buffer_loaded = true;
      }
      uint16_t rpm = (5000 / 240) * (old_angle + 120);
...
// =======================================================================================
  void pngDraw(PNGDRAW *pDraw) {
    ...
    //static uint16_t dmaBuffer[MAX_IMAGE_WDITH];
